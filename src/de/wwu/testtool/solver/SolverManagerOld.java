...
public class SolverManagerOld implements SolverManager {
	...
	for (SolverManagerListener listener : solverConf.getListeners()){
	    ...
	    logger.debug("SolverManager: added listener " + listener.getClass().getName());
	}
    }
	...
	listeners.fireGetSolutionFinished(this, Solution.NOSOLUTION, System.nanoTime() - startTime);
	...
	if (solution != null){
	    ...
	    listeners.fireInternalGetSolutionFinished(this, idx, solution, System.nanoTime() - startTime);
	    return solution;
	}
	...
	if (system.isContradictory()) {
		...
		for (int i = 0; i < system.getConstraintSetCount(); i++){
			...
			for (int solverNo = 0; solverNo < solver.length; solverNo++){
			    ...
			    listeners.fireSolverGetSolutionStarted(this, solver[solverNo], constraintSet);
			    ...
			    listeners.fireSolverGetSolutionFinished(this, solver[solverNo], constraintSet, newSolution, System.nanoTime() - startTime);
			}
		    }
...
//			listeners.fireInternalGetSolutionFinished(this, idx, Solution.NOSOLUTION, System.nanoTime() - startTime);
//			return Solution.NOSOLUTION;
		    }
		}
	...
	if (constraintStack.getSystemCount() == 0){
	    listeners.fireHasSolutionFinished(this, true, System.nanoTime() - startTime);
	    return true;
	}
	...
	listeners.fireHasSolutionFinished(this, false, System.nanoTime() - startTime);
	return false;
    }
	...
	if (solutionTmp != null){
	    ...
	    listeners.fireInternalHasSolutionFinished(this, idx, result, System.nanoTime() - startTime);
	    return result;
	}
	...
	if ( system.isContradictory() ){
	    constraintStack.setSolution(idx, Solution.NOSOLUTION);
	    listeners.fireInternalHasSolutionFinished(this, idx, false, System.nanoTime() - startTime);
	    return false;
	} else {
	    boolean result = true;
	    try {
		for (int i = 0; i < system.getConstraintSetCount(); i++){
			...
			for (int solverNo = 0; solverNo < solver.length && result; solverNo++){
			    ...
			    try {
				listeners.fireSolverHasSolutionStarted(this, solver[solverNo], constraintSet);
				for (SingleConstraint constraint : constraintSet)
				    solver[solverNo].addConstraint(constraint);
				...
				listeners.fireSolverHasSolutionFinished(this, solver[solverNo], (intermediateResult != null)?intermediateResult.hasSolution():false, System.nanoTime() - startTime);
			    }
			}
		    }
	...
	listeners.fireInternalHasSolutionFinished(this, idx, true, System.nanoTime() - startTime);
	return true;
    }
