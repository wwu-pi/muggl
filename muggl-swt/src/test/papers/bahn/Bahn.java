...
public class Bahn {
	...
	public TravelPrice calculateTravelPrice (
			...
			String destination,
			int days) {
		ArrayList<TravelPrice> tpList = new ArrayList<TravelPrice>();
		if (train == 0 && classNo == 2) {
			if (weekendIncluded && days == 1) tpList.add(weekendPrice(persons));
			if (insideNRW && days == 1 && (weekendIncluded || startTime >= 9)) tpList.add(schoenerTagNRW(persons));
			if (insideNRW && days == 1 && duration <= 120) tpList.add(schoeneFahrtNRW(persons));
		}
		if (daysBeforeTravel >= 3 && !flexible && classNo == 2) {
			tpList.add(new TravelPrice("DauerSpezial", dauerSpezial(persons, returnTicket)));
		}
		...
		tpList.add(normalPriceTp);
		
		if (classNo == 2 && train >= 1 && days == 1 && departureFrom == "Hamburg" && destination == "Bremen") { 
			tpList.add(new TravelPrice("SpecialOfferHH2HB",19.0));
		}
		
		// Determine the cheapest price and return that connection.
		TravelPrice cheapest = null;
		for (TravelPrice current : tpList)
		{
			if (cheapest == null || cheapest.getPrice() > current.getPrice()) cheapest = current;
		}
		return cheapest;
